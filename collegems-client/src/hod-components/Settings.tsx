      ...
      const fetchSettings = async () => {
        try {
          ...
          const [profileRes, settingsRes, labelsRes] = await Promise.all([
            api.get("/users/me"),
            api.get("/users/me/preferences").catch(() => ({ data: {} })),
            api.get("/settings"),
          ]);
    ...
    const handleLabelSave = async () => {
      try {
        ...
        await api.put("/settings", {
          academicLabels: labelSettings,
        });
        setMessage({
          type: "success",
          text: "Academic labels updated successfully.",
        });
      } catch (error: any) {
        setMessage({
          type: "error",
          text:
            ...
            "Failed to update academic labels.",
        });
      } finally {
        setSaving(false);
      }
      ...
      <div className="space-y-6">
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <label className="space-y-1 text-sm text-gray-600">
              {getAcademicLabel("department", academicLabels)} Code
              <input
                type="text"
                value={profile.departmentCode}
                onChange={(event) => handleProfileChange("departmentCode", event.target.value)}
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </label>
          </div>
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Object.entries(labelSettings).map(([key, value]) => (
              <label key={key} className="space-y-1 text-sm text-gray-600">
