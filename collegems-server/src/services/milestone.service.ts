 ...
 */
export const getStudentMilestones = async (
  studentId: string,
  options: MilestoneOptions = {}
): Promise<PaginatedMilestones> => {
  const { page = 1, limit = 20, category, status, sort = "desc" } = options;
  ...
  for (const ev of timelineEvents) {
    addMilestone(
      ...
      "Semester",
      "Completed",
      ev.timestamp || (ev as any).createdAt,
  ...
  for (const assign of assignments) {
    const submission = assign.submissions?.find(
      (s: any) => s.student?.toString() === studentId.toString()
    );
    ...
    let assignStatus: Milestone["status"] = "Upcoming";
  ...
  for (const exam of examSchedules) {
    ...
    const examStatus: Milestone["status"] = isPast ? "Completed" : "Upcoming";
  ...
  for (const fee of fees) {
    if (fee.scheduledInstallments && fee.scheduledInstallments.length > 0) {
      for (const inst of fee.scheduledInstallments) {
        let feeStatus: Milestone["status"] = "Upcoming";
        let feeColor = "blue";
  ...
  for (const leave of leaves) {
    let leaveStatus: Milestone["status"] = "Upcoming";
    let leaveColor = "blue";
  ...
  for (const ach of achievements) {
    let achStatus: Milestone["status"] = "Upcoming";
    let achColor = "blue";
  ...
  const limitNumber = Math.max(1, limit || 20);
